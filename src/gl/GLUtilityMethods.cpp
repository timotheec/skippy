...
inline int __GetOpenGLError(char *szFile, int iLine) {
  ...
  while (glErr != GL_NO_ERROR) {
    std::cout << "GLError in file << " << szFile << " @ line " << iLine << ":"
...
void coutBasisFunctions(unsigned int N) {
  ...
  for (unsigned int k = 0; k <= N; ++k) {
    for (unsigned int j = 0; j <= N - k; ++j) {
      std::cout << "    { "
                << (1.f - (float)(j) / (float)(N) - (float)(k) / (float)(N))
                << "   ,   " << ((float)(j) / (float)(N)) << "   ,   "
                << ((float)(k) / (float)(N)) << " }," << std::endl;
      ++nPoints;
    }
  }
  std::cout << "};" << std::endl;
...
void getBarycentricCoordinates(unsigned int N_splits_on_each_edge,
  ...
  x = (1.f - (float)(j) / (float)(N_splits_on_each_edge) -
       (float)(k) / (float)(N_splits_on_each_edge));
...
void get_random_RGB_from_HSV(float &r, float &g, float &b) {
  QColor c =
      QColor::fromHsvF((float)(rand()) / (float)(RAND_MAX),
...
int optimalSlices(float radius, float referenceRadius) {
  return std::min((int)(15),
                  std::max((int)(3), (int)(10 * radius / referenceRadius)));
}
...
void drawSphere(float x, float y, float z, float radius, int slices,
  ...
  for (int i = 1; i <= stacks; i++) {
    ...
    for (int j = 1; j <= slices; j++) {
      ...
      points[j + (i - 1) * slices] =
          qglviewer::Vec(cosT * cosP, sinT * cosP, sinP);
    }
  }
