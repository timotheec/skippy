...
bool openTriMesh(const std::string &filename, std::vector<point_t> &vertices,
                 std::vector<tri_t> &faces, bool randomize = false) {
  std::ifstream myfile;
...
bool save(const std::string &filename, std::vector<point_t> &vertices,
  ...
  while (myfile.good()) {
    ...
    if (lineElements.size() > 0) {
      ...
      if (elementType == QString("v")) {
        vertices.push_back(point_t(lineElements[1].toDouble(),
                                   lineElements[2].toDouble(),
                                   lineElements[3].toDouble()));
      }
      // face
      else if (elementType == QString("f")) {
        std::vector<unsigned int> vhandles;
        for (int i = 1; i < lineElements.size(); ++i) {
          QStringList FaceElements =
              lineElements[i].split("/", QString::SkipEmptyParts);
          if (FaceElements.size() > 0)
            vhandles.push_back(
                (unsigned int)((abs(FaceElements[0].toInt()) - 1)));
        }
...
bool save(std::string const &filename, std::vector<point_t> &verticesP,
  ...
  for (unsigned int v = 0; v < verticesP.size(); ++v) {
    myfile << "v " << (verticesP[v][0]) << " " << (verticesP[v][1]) << " "
           << (verticesP[v][2]) << std::endl;
  }
